...
statements:
  - entity: Q1
    ...
    qualifiers:
      - property: P2
        value: { type: date, value: "2023-12-31" }
    references: [R1]
```
...
- `qualifiers`: optional typed context for the value; each property/value pair may appear only once
...
The manifest format is strict. `entity`, `property`, `value`, and `references` are required; `references` cannot be empty; and unknown fields are rejected. Qualifiers must be valid for the target entity and main property.
...
A statement is already present when the same entity has the same property, typed value, and qualifiers. Qualifier ordering does not matter, but a changed qualifier creates a distinct statement. Changing only references does not create a new statement. Duplicate results use `outcome: not_applied` and `status: already_present`.
