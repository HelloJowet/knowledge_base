...
Filters use `P<n>=value` syntax. The command reads the property definition and interprets the value using its declared `value_type`; see the data model's [supported values](../data-model.md#values). The first `=` separates the property from the value, so string and URL values may contain additional equals signs. Coordinates use `latitude,longitude` syntax; quantities use `amount,unit` syntax.
