...
Every knowledge base also has an `extensions.yaml` file. A base-only knowledge base uses:
...
Keep this file even when no extensions are enabled. Extensions add repository-specific rules and configuration; see the [CLI documentation](cli/README.md#extensions) for checking the active configuration.
...
`extensions.yaml` is required at the knowledge-base root. `version` must be `1`; `extensions` defaults to an empty map when omitted. Each enabled extension declares its supported `contract` version and maps its named entity-type and property bindings to local IDs:
...
The executable must include every enabled extension. Its extension-specific requirements and bindings must also be valid for the current repository.
